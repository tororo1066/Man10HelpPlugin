        ...
        if (command.label == "mhelp"){
            if (args.isEmpty()) {
                ...
                if (sender.hasPermission("admin")){
                    ...
                    sender.sendMessage("§a/mhelp set (Int(0~)) (title) (subtitle) ムービーの場所、")
                    sender.sendMessage("§aタイトル、順番を指定できます")
                    sender.sendMessage("§a/mhelp reload configをリロードします(誰かがムービーを見てるとできません)")
                    sender.sendMessage("§a/mhelp showsetting ムービーの設定ができます")
            ...
            when(args[0]){
                ...
                "set"->{
                    ...
                    plugin.config.set("show.$n.txt",title)
                    plugin.config.set("show.$n.subtxt",subtitle)
                    plugin.config.set("show.$n.loc",sender.location)
                ...
                "showsetting"->{
                    ...
                    if (args.size == 1){
                        sender.sendMessage("§a====================Man10HelpPlugin====================")
                        sender.sendMessage("§akaisuu (Int) タイトルの表示回数を指定します")
                        sender.sendMessage("§atitle (Int) タイトルの表示時間をtick単位で指定します")
                        sender.sendMessage("§afadein (Int) タイトルのフェードインの時間をtick単位で指定します")
                        sender.sendMessage("§afadeout (Int) タイトルのフェードアウトの時間をtick単位で指定します")
                        sender.sendMessage("§afirst (title) (subtitle) 最初のタイトルを設定します")
                        sender.sendMessage("§alast (title) (subtitle) 最後のタイトルを設定します")
                        sender.sendMessage("§a====================Man10HelpPlugin====================")
                    }else {
                        when (args[1]) {
                            ...
                            "kaisuu" -> {
                                ...
                                plugin.config.set("show.kaisuu", n)
                                sender.sendMessage(prefix + "コンフィグへの書き込みが完了しました")
                                sender.sendMessage("$prefix/mhelp reloadで設定を変更してください")
                                return true
                            ...
                            "title" -> {
                                ...
                                plugin.config.set("show.title", n)
                                sender.sendMessage(prefix + "コンフィグへの書き込みが完了しました")
                                sender.sendMessage("$prefix/mhelp reloadで設定を変更してください")
                                return true
                            ...
                            "fadein" -> {
                                ...
                                plugin.config.set("show.fadein", n)
                                sender.sendMessage(prefix + "コンフィグへの書き込みが完了しました")
                                sender.sendMessage("$prefix/mhelp reloadで設定を変更してください")
                                return true
                            }
                            ...
                            "fadeout" -> {
                                ...
                                plugin.config.set("show.fadeout", n)
                                sender.sendMessage(prefix + "コンフィグへの書き込みが完了しました")
                                sender.sendMessage("$prefix/mhelp reloadで設定を変更してください")
                                return true
                            }
                            "first" -> {
                                ...
                                val title = args[2].replace("&","§")
                                val subtitle = args[3].replace("&","§")
                                plugin.config.set("show.first.txt", title)
                                plugin.config.set("show.first.subtxt", subtitle)
                                plugin.config.set("show.first.loc", sender.location)
                                sender.sendMessage(prefix + "コンフィグへの書き込みが完了しました")
                                sender.sendMessage("$prefix/mhelp reloadで設定を変更してください")
                                return true
                            }
                            ...
                            "last" -> {
                                ...
                                val title = args[2].replace("&","§")
                                val subtitle = args[3].replace("&","§")
                                plugin.config.set("show.last.txt", title)
                                plugin.config.set("show.last.subtxt", subtitle)
                                plugin.config.set("show.last.loc", sender.location)
                                sender.sendMessage(prefix + "コンフィグへの書き込みが完了しました")
                                sender.sendMessage("$prefix/mhelp reloadで設定を変更してください")
                                return true
                            }
